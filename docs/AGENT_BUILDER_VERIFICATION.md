...
| **Workflow Orchestrator**         | ✅ Implemented | `server/services/AWS-orchestrator.service.ts` | Event pipeline automation (503 lines)               |
...
- ✅ Publish to Amazon SQS + SNS
- ✅ Notify organizers via Amazon SNS Push
...
// Step 3: Calculate optimal routes using Amazon Location Service
...
2. Video Analytics publishes FIRE alert to Amazon SQS + SNS
3. Alert triggers Agent Builder createDispatchPlan():
   ...
   - Amazon Location Service calculates fastest routes
   - Teams dispatched with ETA: 3 minutes
4. Amazon SNS Push notifications sent to:
...
### Location: `server/services/AWS-orchestrator.service.ts` (503 lines)

#### **Purpose**: Connect all AWS tools in automated workflows
...
interface EventDataPipeline {
  ...
  delivery: {
    ...
    notifications: boolean; // Send Amazon SNS Push push notifications
    routing: boolean; // Recalculate routes (Amazon Location Service)
  };
}
```
...
async processEventData(eventId: string, dataType: string, data: any): Promise<void> {
  // 1. Ingest data via Amazon SQS + SNS (IF: New data arrives)
  ...
  if (pipeline.ml.forecasting && dataType === 'CROWD_DENSITY') {
    // Trigger Amazon SageMaker forecasting (5-30 min ahead)
  }
  ...
  // 4. Update real-time databases (THEN: Amazon DynamoDB for dashboards)
  if (pipeline.delivery.dashboards) {
    await Amazon Cognito+S3AdminService.setDocument({
  ...
  // 5. Send emergency alerts (THEN: Amazon SNS Push notifications, auto-dispatch)
  if (anomalyDetected) {
...
router.post('/create', authenticate, requireRoles(['ADMIN', 'SECURITY', 'ORGANIZER']), async (req, res) => {
  ...
  // 5. Publish to Amazon SQS + SNS for downstream processing
...
└─────────────────────────────────────────────────────────────────┘
  ...
  5. Amazon SQS + SNS publishes alert → Amazon SNS Push notifications
...
└─────────────────────────────────────────────────────────────────┘
  ...
  4. Amazon SQS + SNS publishes to 'anomaly-events' topic
...
└─────────────────────────────────────────────────────────────────┘
  ...
  6. Dispatch Plan created:
     ...
     - Calculate routes using Amazon Location Service
...
└─────────────────────────────────────────────────────────────────┘
  8. Amazon SNS Push notifications sent:
...
- [x] Workflow Orchestrator (AWS services coordination)
- [x] Dispatch API routes (`/api/dispatch/create`, `/api/dispatch/:id/approve`)
- [x] Amazon SQS + SNS event streaming (alerts, dispatches)
- [x] Amazon Athena logging (audit trail)
- [x] Amazon SNS Push notifications (organizers, teams, attendees)
...
- [x] Data ingestion (drones, CCTV, GPS, SageMaker Geospatial)
- [x] Real-time processing (Amazon SQS + SNS, ETL pipeline)
...
- [x] Audit logging (all actions tracked in Amazon Athena)
...
5. **Real-Time Integration**: Amazon SQS + SNS, Amazon DynamoDB, Amazon SNS Push for instant updates
6. **Frontend UX**: Clear auto-dispatch UI with USP 4 branding
7. **Audit Trail**: All actions logged to Amazon Athena for compliance
...
├──────────────────────────────────────────────────────────────────────────┤
│  Drones  │  CCTV  │  GPS  │  SageMaker Geospatial  │  Social Media  │  Weather  │
└────┬─────┴───┬────┴───┬───┴────────┬───────┴────────┬───────┴───────┬───┘
     │         │        │            │                │               │
     └─────────┴────────┴────────────┴────────────────┴───────────────┘
                                     │
                                     ▼
┌──────────────────────────────────────────────────────────────────────────┐
│                    STREAMING LAYER (Amazon SQS + SNS Topics)                      │
├──────────────────────────────────────────────────────────────────────────┤
...
├──────────────────────────────────────────────────────────────────────────┤
│  AWS App Runner ETL Worker  │  Data Cleaning  │  Feature Engineering          │
└────┬───────────────────┴─────────────────┴───────────────────────────────┘
     │
     ▼
┌──────────────────────────────────────────────────────────────────────────┐
...
3. **Workflow Orchestrator** (AWS services coordination)
