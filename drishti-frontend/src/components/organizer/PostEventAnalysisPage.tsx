...
// Mock post-event data
const INCIDENT_STATS: IncidentStats = {
  total: 47,
  resolved: 42,
  unresolved: 5,
...
const PERFORMANCE_METRICS: PerformanceMetric[] = [
  {
    ...
    score: 89,
    status: 'excellent',
    details: 'Average response time: 3.2 mins (Target: <5 mins)'
  },
  {
    category: 'Crowd Management',
    score: 76,
    status: 'good',
    details: 'Peak capacity managed at 85% without issues'
  },
  {
    category: 'Gate Operations',
    score: 92,
    status: 'excellent',
    details: 'Smooth entry/exit flow, minimal bottlenecks'
  },
  {
    category: 'Medical Response',
    score: 65,
    status: 'needs-improvement',
    details: 'Two incidents exceeded target response time'
  },
  {
    category: 'Communication',
    score: 88,
    status: 'excellent',
    details: 'Real-time alerts delivered to all stakeholders'
  }
];

const AI_ACCURACY: AIAccuracy[] = [
  {
    metric: 'Crowd Density Prediction',
    predicted: 12500,
    actual: 12847,
    accuracy: 97.3
  },
  {
    metric: 'Peak Time Forecast',
    predicted: 20.75, // 20:45 as decimal
    actual: 20.75,
    accuracy: 100
  },
  {
...
export function PostEventAnalysisPage({ onBack }: PostEventAnalysisPageProps) {
  const [selectedMetric, setSelectedMetric] = useState<string | null>(null);
  const [eventEnded] = useState(true); // Simulate event status
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'excellent': return <Award className="w-5 h-5 text-emerald-600" />;
      case 'good': return <ThumbsUp className="w-5 h-5 text-blue-600" />;
      case 'needs-improvement': return <ThumbsDown className="w-5 h-5 text-amber-600" />;
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-slate-600">
            Post-Event Analysis will be available once the event has concluded. 
            Check back after the event ends to view comprehensive analytics and insights.
          </p>
          <button 
            onClick={onBack}
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            ...
            <p className="text-3xl font-bold text-slate-900">{INCIDENT_STATS.total}</p>
            <p className="text-xs text-slate-500 mt-1">{INCIDENT_STATS.criticalIncidents} critical</p>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            ...
            <p className="text-3xl font-bold text-emerald-600">{INCIDENT_STATS.resolved}</p>
            <p className="text-xs text-slate-500 mt-1">{((INCIDENT_STATS.resolved / INCIDENT_STATS.total) * 100).toFixed(1)}% resolution rate</p>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            ...
            <p className="text-3xl font-bold text-red-600">{INCIDENT_STATS.unresolved}</p>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            ...
            <p className="text-3xl font-bold text-blue-600">{INCIDENT_STATS.avgResponseTime}</p>
            ...
            {PERFORMANCE_METRICS.map((metric, idx) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className={`flex flex-col items-center px-4 py-2 rounded-lg border-2 ${getScoreBgColor(metric.score)}`}>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-slate-900">
                        {typeof item.predicted === 'number' && item.predicted > 100 ? item.predicted.toLocaleString() : item.predicted}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-600">Actual</span>
                      <span className="font-semibold text-slate-900">
                        {typeof item.actual === 'number' && item.actual > 100 ? item.actual.toLocaleString() : item.actual}
                      </span>
                    </div>
            ...
            <div className="mt-4 p-4 bg-emerald-50 border border-emerald-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-emerald-800">
                  <span className="font-semibold">Overall AI Performance:</span> The AI prediction engine demonstrated strong accuracy across all metrics, with an average accuracy of 95.1%. This validates the effectiveness of the AI-driven approach for event management.
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3 p-3 bg-emerald-50 rounded-lg border border-emerald-200">
              ...
              <div>
                ...
                <p className="text-sm text-emerald-700">
                  Excellent incident response times, smooth gate operations, and effective crowd management at peak capacity.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 p-3 bg-amber-50 rounded-lg border border-amber-200">
              ...
              <div>
                ...
                <p className="text-sm text-amber-700">
                  Medical response times exceeded targets in two instances. Consider increasing medical team capacity for future events.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700">
                  Deploy 2 additional medical teams for events with similar attendance. Maintain current staffing levels for security and operations.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-slate-100 border border-slate-300 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-700">
              This analysis is generated from event data and AI insights. All metrics and recommendations are available for export and can be used for future event planning.
            </p>
          </div>
        </div>
      </div>
