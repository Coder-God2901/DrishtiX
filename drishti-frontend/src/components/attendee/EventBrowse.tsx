...
export function EventBrowse({ onBack, onEventSelect }: EventBrowseProps) {
  ...
  const [sortBy, setSortBy] = useState<'date' | 'price' | 'rating' | 'attendance'>('date');
  ...
  useEffect(() => {
    ...
    wsService.on('attendance:update', handleAttendanceUpdate);

    return () => {
      wsService.off('event:updated', handleEventUpdate);
      wsService.off('event:created', handleEventUpdate);
      wsService.off('attendance:update', handleAttendanceUpdate);
  ...
  const loadEvents = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        // Convert API events to UI events - use real backend data
        const convertedEvents: Event[] = response.data.map((e) => ({
          ...e,
          date: e.startTime,
          venue: e.venue || e.location,
          expectedAttendance: e.expectedAttendees,
          currentAttendance: e.actualAttendees,
          safetyScore: (e as any).safetyScore,
          category: (e as any).category || 'Event',
          price: (e as any).ticketPrice || 0,
          rating: (e as any).rating,
          averageRating: (e as any).averageRating,
          queueTime: (e as any).queueTime,
          image: (e as any).imageUrl || '/event-placeholder.jpg',
          time:
            typeof e.startTime === 'string'
              ? new Date(e.startTime).toLocaleTimeString()
              : e.startTime.toLocaleTimeString(),
          isFree: (e as any).ticketPrice === 0 || (e as any).isFree,
          crowdStatus: (e as any).crowdStatus,
          wheelchairAccessible: (e as any).wheelchairAccessible,
          parkingAvailable: (e as any).parkingAvailable,
          foodVendors: (e as any).foodVendors,
        }));
  ...
  const filteredEvents = events
    .filter((event) => {
      ...
      if (selectedFilters.includes('today') && event.date !== 'Today') return false;
      if (selectedFilters.includes('free') && !event.isFree) return false;
      if (selectedFilters.includes('paid') && event.isFree) return false;
      if (selectedFilters.includes('calm') && event.crowdStatus !== 'calm') return false;
      if (selectedFilters.includes('music') && event.category !== 'Music') return false;
      if (selectedFilters.includes('tech') && event.category !== 'Technology') return false;
      if (selectedFilters.includes('accessible') && !event.wheelchairAccessible) return false;
      if (selectedFilters.includes('nearby') && parseFloat(event.location) > 5) return false;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/30 to-purple-50/20">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {/* Featured Events */}
        {featuredEvents.length > 0 && selectedFilters.includes('all') && !searchQuery && (
          <div className="mb-8">
              ...
              >
                ...
                <div className="relative h-48">
                  ...
                  <div className="absolute top-3 left-3 flex gap-2">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${getCrowdStatusColor(event.crowdStatus)}`}
                    >
                ...
                <div className="p-5">
                    ...
                    {event.currentAttendance && (
                      <div className="flex items-center gap-2 text-slate-600 text-sm">
                        ...
                        <span className="text-xs text-green-600 font-medium">
                          {Math.round((event.currentAttendance / (event.expectedAttendance || 1000)) * 100)}%
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    <div className="bg-green-50 rounded-lg p-2 text-center">
                      ...
                      <p className="text-lg font-bold text-green-700">{event.safetyScore}</p>
                    </div>
                    <div className="bg-yellow-50 rounded-lg p-2 text-center">
                      ...
                      <p className="text-lg font-bold text-yellow-700">{event.averageRating}</p>
                    </div>
                    <div className="bg-blue-50 rounded-lg p-2 text-center">
                      <div className="flex items-center justify-center gap-1 mb-1">
                        <Clock className="w-3 h-3 text-blue-600" />
                        <p className="text-xs text-blue-600 font-medium">Queue</p>
                      </div>
                      <p className="text-lg font-bold text-blue-700">{event.queueTime}m</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-200">
                    <div>
                      {event.isFree ? (
                        ...
                        <div>
                          <p className="text-2xl font-bold text-slate-900">₹{event.price}</p>
