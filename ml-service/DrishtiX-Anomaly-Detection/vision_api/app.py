...
def analyze_frame():
    """
    API endpoint for the DrishtiX Stream Engine. 
    It captures a single frame from the active camera, runs the model, 
    and returns a JSON assessment.
    """
    global camera, camera_active, model
    
    # If the camera isn't running, we can't capture a frame
    if not camera_active or camera is None:
        return jsonify({
            'success': False, 
            'error': 'Camera is offline. Start camera via web interface first.'
        }), 503
    ...
    try:
        ...
        success, frame = camera.read()
        if not success:
            return jsonify({'success': False, 'error': 'Failed to grab frame from camera.'}), 500
            
        # Convert BGR to RGB exactly like you did in the video processor
        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        
        # Run the OpenAI CLIP model prediction
        result = model.predict(rgb_frame)
        label = result['label']
        confidence = result['confidence']
        category = get_category(label)
        
        # Build a human-readable description for the LLM
        if category == 'Normal':
            description = "No anomaly detected. Scene appears normal."
        else:
            description = f"Detected {label} ({category}) with {confidence*100:.1f}% confidence."
